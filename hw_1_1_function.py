...
class Network:
    ...
    def train(self, train_data, labels, y1_train , test_data , test_Labels ,all_image_data , all_label_data):
        ...
        crosstab_test_data = self.crosstab_cal(test_data, test_Labels)
        crosstab_all_data = self.crosstab_cal(all_image_data, all_label_data)
        return crosstab_test_data , crosstab_all_data
    ...
    def crosstab_cal(self, test_data, labels):
        input_layer = np.dot(test_data, self.weight1)
        hidden_layer = relu(input_layer + self.bias1)
        scores = np.dot(hidden_layer, self.weight2) + self.bias2
        probs = softmax(scores)

        crosstab = pd.crosstab(labels , np.argmax(probs, 1) ,rownames= ['labels'] , colnames=['predict'])
